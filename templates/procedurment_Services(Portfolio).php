    ...
    <div class="container mx-auto px-4">
        ...
        <div class="about_us">
            ...
            foreach($procurement_services_supply_chain['chain'] as $chain):
                if (empty($chain['image']['url']) && empty($chain['heading']) && empty($chain['description'])) continue;
            ?>
                <div class="bg-white rounded-[20px]  px-[10px] py-[20px] md:px-[32px] md:py-[40px]  flex flex-col justify-center items-center">
                    ...
                    <p class="custom-ul-ol roboto text-[14px] text-[#000000] font-normal text-center text-black tracking-[0.84px]">
    ...
    <div class="container mx-auto px-4">
        ...
        <?php 
            foreach($procurement_services_major_procurement['major_projects'] as $major_procurement_alls):
                if (empty($major_procurement_alls['image']['url']) && empty($major_procurement_alls['heading']) && empty($major_procurement_alls['description'])) continue;
            ?>
            <div class="used">
                <?php if (!empty($major_procurement_alls['image']['url'])) : ?>
                <img src="<?php echo esc_url($major_procurement_alls['image']['url']); ?>" height="336" class="rounded-t-[20px] w-full">
                <?php endif; ?>
                <div
    ...
    <div class="container mx-auto px-4">
        ...
        <div class="allpartners grid !grid-cols-2 lg:!grid-cols-4 gap-[20px] md:!grid-cols-2">
            ...
            foreach($procurement_services_key_partnerships['partners'] as $partner):
                if (empty($partner['image']['url'])) continue;
            ?>

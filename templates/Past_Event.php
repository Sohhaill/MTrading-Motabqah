    ...
    <div class="Past_event">
      <div class="form_text mb-[40px] w-full md:w-[486px]">
        <?php if (!empty($summery_heading)): ?>
          <h1
            class="font-montserrat uppercase text-[20px] text-[#05060F] md:text-[32px] font-bold leading-[24px] md:leading-[40px] tracking-[3.8px] mb-[24px]">
  ...
  <div class="flex flex-col gap-[20px]">
    <?php foreach ($summery_all_events as $past): ?>
      <div class="container mx-auto px-4 flex flex-col lg:flex-row rounded-[20px] gap-y-[20px]">
        <div class="apply bg-white md:py-[40px] !px-[20px] md:pl-[32px] md:pr-[20px] rounded-l-[20px] w-[100%] lg:w-[55%]">
          <?php if (!empty($past['heading'])): ?>
            <h1 class="uppercase roboto text-[20px] md:text-[24px] font-bold tracking-[1.2px]  mb-[8px]">
          ...
          <div class="applytext">
            ...
            <?php foreach ($past_information as $past_info): ?>
              <?php if (!empty($past_info['heading']) || !empty($past_info['description'])): ?>
                <div class="grid grid-cols-1 md:grid-cols-3 mt-[12px] gap-x-[30px]">
                  ...
                  <?php if (!empty($past_info['description'])): ?>
                    <p class="roboto text-[14px] text-[#000000] font-normal leading-[20px] text-black tracking-[0.84px] col-span-2">
        ...
        <?php if (!empty($past['image']['url'])): ?>
          <img src="<?php echo esc_url($past['image']['url']); ?>"
            class=" w-[100%] lg:w-[45%] h-[unset]  rounded-tr-[20px]">
        <?php endif; ?>
      </div>
    <?php endforeach ?>
  </div>
